...
def DAGs_generate(mode = 'default', n = 10, max_out = 2,alpha = 1,beta = 1.0):
	...
	for node,id in enumerate(into_degree):# add start node
		if id ==0:
			edges.append(('Start',node+1))
			into_degree[node]+=1

	for node,od in enumerate(out_degree):# add exit node
		if od ==0:
...
def plot_DAG(edges, postion, dep_data, CL, var=0.2):
	g1 = nx.DiGraph()
	dep_data_list = []
	...
	for i, item in enumerate(edges):
		item = item + (dep_data_list[i],)
		weighted_edges.append(item)
	...
	for i, node in enumerate(g1.nodes()):
		#print(i, node)
		g1.nodes[node]['CL'] = CL[i]

	nx.draw_networkx(g1, arrows=True, pos=postion)
	edge_labels = nx.get_edge_attributes(g1, 'weight')
	nx.draw_networkx_edge_labels(g1, pos=postion, edge_labels = edge_labels)
...
def gen_application(noT, CL_avg, dep_data, var):
	edges,in_degree,out_degree,position = DAGs_generate('default', n=noT-2)
	# print(edges)
	...
	G = plot_DAG(edges, position, dep_data, CL, var)

	print(list(nx.bfs_tree(G, 'Start')))

	return G


if __name__ == '__main__':
	gen_application(12, 300, 6000, 0.2)
